...
requires filling in game-specific signatures/offsets** — the LEAD engine internals are not
public. Recommended order (lowest → highest risk):

1. **Object Dumper first** (read-only). Adjust the node-layout offsets in the Discovery panel
   and *Dump types* until the console prints recognizable type names (`Actor`, `Mesh`, …).
   Then *Resolve catalog* — the browser entries should flip to `[live]`.
2. **Spawn factory.** Fill `SPAWN_BY_TYPE_SIG` in `mod/features/engine_bridge.h` with the
   signature of the LEAD "instantiate type" factory. Verify the function prologue in a
   disassembler before the first call. Because the types are data-defined, this is expected to
   be **one** factory, not 62 constructors.
...
mod/
  ...
  features/
    ...
    object_dumper.*      — Locates the LEAD type registry via type-name string
                           anchors; walks it to resolve live type-node pointers
